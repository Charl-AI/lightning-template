...
class _DummySegmentationData(Dataset):
    """Dummy segmentation data. The data consists of random images
    (with a Gaussian distribution) and the labels are generated by
    thresholding the pixels at 0"""
    ...
    def __init__(self, img_shape: tuple = (1, 100, 100), length: int = 10):
        ...
        self.targets = torch.where(
            self.imgs > 0, torch.ones_like(self.imgs), torch.zeros_like(self.imgs)
        )
...
class DummySegmentationDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if stage == "test" or stage is None:
            self.test = _DummySegmentationData(
